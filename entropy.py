...
def filtration_entropy(X, tail):
    ...
    dist_matrix = np.zeros((len(X), len(X)))  # 用来存储距离矩阵

    tic_matrix = time.time()
    for i in np.arange(len(X)):
        for j in np.arange(len(X)):
            dist_matrix[i][j] = np.linalg.norm(X.iloc[i] - X.iloc[j], ord=2)  # 2-norm
    toc_matrix = time.time()
    print('Matrix time consuming:' + str(toc_matrix - tic_matrix) + 's')

    length = (len(X) - 1) * len(X) / 2  # 只用存储距离矩阵上对角的元素，剔除对角线上的全0值
